...
int handle_open_bracket(char last_portion[], int command_flag, int index_of_tok, struct sst *res){
	...
	if(operand_label_errors(current_portion, res) != FOUND_ERROR) {
		strcpy(res->asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.label, current_portion);
		return 1;
	}
	else
		return FOUND_ERROR;
...
int handle_close_bracket(int command_flag, int num_of_operands, int close_bracket, int open_bracket, char last_portion[], int index_of_tok, struct sst *res) {
	...
	/*Checking if the operand is a label.*/	
		...
		strcpy(res->asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.operands[num_of_operands].label, 			current_portion);
	}
	...
	switch(determine_set(command_flag)) {
	   case SET1:
		...
		/*The second operand is a label.*/
			...
			res-> asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_2_operands.ops_tags[num_of_operands] = sst_tag_op_label;
			strcpy(res->asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_2_operands.operands[num_of_operands].label,last_portion);
		}
	   ...
	   case SET2:
		...
		if(num_of_operands == 0) {
			...
			/*Operand is a label*/
				...
				strcpy(res->asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.operands.label, last_portion);	
			}
