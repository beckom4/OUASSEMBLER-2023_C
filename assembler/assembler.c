...
#define PROG_START_ADDR 100   /* first memory address of program start */


/*----- STRUCTS: -----*/

/*
* Represents label in the symbol table
*/
struct asm_label {
...
/*
* Represents strings of label options:   
*/
...
struct CU {
    ...
    int *   bmc_code_list; /* code list, used in bmc */
    int *   bmc_data_list; /* data list, used in bmc*/
    int *   ref_to_entry_symbols;  /* pointer to refrence array to entry symbols address in the symbol table. */
    struct extern_use *  extern_list;  /* extern list  */
};


/* ----- FUNCTIONS IMPLAMANTATIONS: ----- */
...
struct extern_use * assembler_extern_use_search(struct extern_use * extern_use_arr, const char * label) {
    size_t i;
...
/**
* Search given label in a given symbol table.
*
* @param symbol_table - pointer to the given symbol table to search in
* @param symbol - pointer to the label of the given symbol needed to search.
**/
...
static int assembler_first_pass(const char *am_file_name,FILE * am_file, struct CU * cu) {
                            ...
                            if(syntax_line.syntax_option == sst_instruction) {
                                switch (find->asm_label_type)
                                {
                                    ...
                                    default: 
                                        ...
                                        assembler_print_error(am_file_name,line_counter,"symbol '%s' was defined in line %d as '%s' and now is being redefined as '%s'.\n",find->label_str,find->line_of_definition,asm_label_type_strings[find->asm_label_type],asm_label_type_strings[find->asm_label_type]);
                                        first_pass_err=1;
                                        break;
                                }
                        }
            ...
            if(syntax_line.syntax_option == sst_instruction) {
                ...
                switch (syntax_line.asm_directive_and_cpu_instruction.syntax_directive.dir_tag)
                {
                    ...
                    case sst_tag_dir_extern: case sst_tag_dir_entry:
                            ...
                            switch (find->asm_label_type)
                             {
                                case asm_label_type_extern:
                                    if(syntax_line.asm_directive_and_cpu_instruction.syntax_directive.dir_tag == sst_tag_dir_entry ) {
                                        assembler_print_error(am_file_name,line_counter,"symbol '%s' was defined in line %d as '%s' and now is being redefined as '%s'.\n",find->label_str,find->line_of_definition,asm_label_type_strings[find->asm_label_type],asm_label_type_strings[asm_label_type_entry]);
                                        first_pass_err =1;
                                    
                                    }
                                    else {
                                        /* warning*/
                                        assembler_print_warning(am_file_name,line_counter,"symbol '%s' was defined in line %d as '%s' and now is being redefined as '%s'.\n",find->label_str,find->line_of_definition,asm_label_type_strings[find->asm_label_type],asm_label_type_strings[asm_label_type_extern]);
                                    }
                                    break;
			            ...
			            new_label.asm_label_type = syntax_line.asm_directive_and_cpu_instruction.syntax_directive.dir_tag == sst_tag_dir_entry ?  asm_label_type_entry :  asm_label_type_extern;
                        new_label.line_of_definition = line_counter;
                        dynarray_push(cu->symbol_table,new_label);
                    }
                    break;
    ...
    for(i=0;i<dynarray_length(cu->symbol_table);i++) {
	        ...
	        if(cu->symbol_table[i].asm_label_type == asm_label_type_data_entry || cu->symbol_table[i].asm_label_type == asm_label_type_code_entry) {
...
/** 
 * assembler second pass on the input file - pass the input file in the second time encode the commands. 
**/
static int assembler_second_pass(const char *am_file_name,FILE * am_file, struct CU * cu) {
    ...
    while(fgets(line_buff,MAX_LINE_LEN,am_file)) {
        ...
        case sst_instruction: {
            /* first machine word*/
            mem_word  = syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.cpu_i_tag << 6; /* op code*/
            if(is_inst_tag_set_A(syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.cpu_i_tag)) {
                ...
                if(syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_2_operands.ops_tags[0] == sst_tag_op_register &&
                                syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_2_operands.ops_tags[1] == sst_tag_op_register) {
                           mem_word = (syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_2_operands.operands[0].reg  << 8) | /* src*/
                    ...
                    for(i=0;i<2;i++) {
                        ...
                        case sst_tag_op_label: /* label */
                            ...
                            if(sym_find->asm_label_type == asm_label_type_extern) {
                                ...
                                if(extern_find) {
                                    i = dynarray_length(cu->bmc_code_list) +PROG_START_ADDR; 
                                    ...
                                    i = dynarray_length(cu->bmc_code_list) +PROG_START_ADDR; 
                                    dynarray_push(new_extern_use.addresses,i);
                                    dynarray_push(cu->extern_list,new_extern_use);
                                }
                            }else {
                ...
                if(syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.ops_tag == sst_tag_op_label_with_operands) { /*  BNE,JSR,JMP case L(p1,p2)*/
                    ...
                    for(i=0;i<2;i++){
                        switch (syntax_line.asm_directive_and_cpu_instruction.
                                instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.ops_tags[i]) {
                        case sst_tag_op_register: /* register */
                            mem_word |= 3 << (12 - i * 2);
                            break;
                        case sst_tag_op_label: /* label */
                    ...
                    if(sym_find != NULL) { /* if we found the symbol*/
                        ...
                        dynarray_push(cu->bmc_code_list,mem_word);
                            ...
                            /* error .... undefined symbol*/
                            assembler_print_error(am_file_name,line_counter,"undefined symbol '%s'.\n",syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.label);
                            second_pass_error=1;
                    }
				    /*If both operands are registers.*/
				    if(syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.ops_tags[0] == sst_tag_op_register &&
                        syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.ops_tags[1] == sst_tag_op_register){
					    mem_word = (syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.
					    set_1_operands.operands.label_with_ops.operands[0].reg << 8) | (syntax_line.asm_directive_and_cpu_instruction.
					    instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.operands[1].reg << 2);
					    dynarray_push(cu->bmc_code_list,mem_word);
				    }
				    else{
		    		    for(i=0;i<2;i++) {
                       			 ...
                       			 {
                        			case sst_tag_op_register: /* register */
                               				/* if it's a register depends if it's source or dest put it in the right bits according to i src(8-(0 * 6)) 
							dest(8-(1 * 6)) */
                              				mem_word = syntax_line.asm_directive_and_cpu_instruction.
			       				instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.operands[i].reg << (8 - (i * 6));
                            				break;
                        			case sst_tag_op_label: /* label */
                           			 	sym_find = assembler_search_symbol(cu->symbol_table, syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.operands.label_with_ops.operands[i].label);
                            				if(sym_find != NULL) { /* if we found the symbol*/
                           					 if(sym_find->asm_label_type == asm_label_type_extern) {
                                					mem_word = 1; /* A.R.E = 01*/
                                					extern_find = assembler_extern_use_search(cu->extern_list,sym_find->label_str);
                                			...
                                			/* error .... undefined symbol*/
                                				assembler_print_error(am_file_name,line_counter,"undefined symbol '%s'.\n",syntax_line.
								                asm_directive_and_cpu_instruction.instruction_syntax.
								                inst_operands.set_1_operands.operands.label_with_ops.operands[i].label);
                                			second_pass_error=1;
                            				}
                           				break;
						case sst_tag_op_c_number: /* constant number*/
                           			 	mem_word = syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.
			    				set_1_operands.operands.label_with_ops.operands[i].c_number << 2;
                            				break;
                        			default:
                           				 break;
                        		}
			    ...
			    dynarray_push(cu->bmc_code_list,mem_word);
                            ...
                            if(sym_find != NULL) { /* if we found the symbol*/
                                ...
                                /* error .... undefined symbol*/
                                assembler_print_error(am_file_name,line_counter,"undefined symbol '%s'.\n",syntax_line.asm_directive_and_cpu_instruction.instruction_syntax.inst_operands.set_1_operands.operands.label);
                                second_pass_error=1;
                            }
                            break;
                        case sst_tag_op_c_number: /* constant number*/
...
static void assembler_print_ext_file(FILE *ext,const struct extern_use * eu) {
    int i,j;
    int extern_use_len , extern_addresses_calls;
    for(i=0,extern_use_len =dynarray_length((void*)eu) ; i<extern_use_len ; i++) {
        for(j=0,extern_addresses_calls = dynarray_length(eu[i].addresses);j<extern_addresses_calls;j++) {
            fprintf(ext,"%s\t\t%d\n",eu[i].extern_name,eu[i].addresses[j]);
        }
    }
}
	        ...
	        am_file = fopen(am_file_name,"r");
                if(am_file != NULL) {
                    ...
                    if(assembler_first_pass(am_file_name,am_file,&current_cu) == 0) {
                        ...
                        if(assembler_second_pass(am_file_name,am_file,&current_cu) == 0) {
                            ...
                            if(dynarray_length(current_cu.bmc_code_list) > 0 || dynarray_length(current_cu.bmc_data_list)) {
                                ...
                                strcat(strcpy(ob_file_name,file_names[i]),".ob");
                            ...
                            if(dynarray_length(current_cu.extern_list) > 0) {
                                ...
                                strcat(strcpy(ext_file_name,file_names[i]),".ext");
                    ...
                    for(j=0;j<dynarray_length(current_cu.extern_list);j++) {
                        dynarray_destroy(current_cu.extern_list[j].addresses);
                    }
